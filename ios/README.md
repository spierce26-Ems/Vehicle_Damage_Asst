...
- [x] **Does the appendix's override note exist at all?** **BUILT (`408a247`, `b0968f3`); everything after "History of the finding" is the record of how it was found, not the current state.** `gateOverridden` is recorded at capture and surfaced in `PhotoReviewView`'s metadata strip and a new `Capture Conditions` appendix page; `frameConfirmedClear` decodes `nil`, so "never asked" is not a declined attestation; `sharpnessScore` is `nil` because not-measured is not measured-as-poor; `isBlurry`/`isTooFar` are set on the scar path from the **measured** sharpness and fill terms — not from the gates. They were originally wired to `!isFocused` and `!isCloseEnough`, which the spec clause literally asked for and which are both `false` before anything is measured, so the appendix rendered *"The app measured this photograph as not sharp"* over an unmeasured frame — and rendered it in the same bullet list as *"Sharpness was not measured for this photograph"*, one paragraph contradicting itself in an evidence appendix. **The clause was corrected in the same commit as the code, because a clause that authorises the defect will re-authorise it.** The Item 2 UX spec is landed at `docs/ITEM2_NORULER_FOCUSGATE_UX_SPEC.md` and the appendix's six citations now name it. §2.4 is built section-level rather than per-factor, with one added subject sentence ratified as `Copy: changed`: `FactorScore` carries no photo linkage, so the bare locked sentence under a factor row would let its *position* assert which photographs fed that factor. The landed copy was verified byte-for-byte against the lock — §2.1's header including its non-optional second sentence, all five §2.2 note strings, §2.3's all-clear line — with conditions tested against reportable state so `nil` and `false` are never collapsed.
  ...
  **Still open, named rather than closed quietly, in the order to take them:** **THE DECLINE AFFORDANCE IS BUILT** — `frameConfirmedClear` was `Bool?` for the three-state, but both Ready buttons were one control whose second tap wrote `true`, so **no code path produced `false`**: §2.2's row two and §1.4's badge were correct as written and could not fire on any build, and four documents called the field three-state while the tree made it two. The question now takes two answers (`Yes — clear` / `No — not clear`), **neither blocking the capture** — a flagged photograph in the file beats a missing one, and an answer that costs the examiner their shot is one nobody gives twice. **The locked wording is untouched; the repair is an affordance, since softening row two to describe an unavailable answer would be row five's mistake committed in copy.** The two answer strings are **locked** (`EVIDENCE_APPENDIX_CAPTURE_NOTES.md` §4.2.1, with ledger entries in §4.1 — including one for `confirm.arm`, whose text did not change but whose meaning did). **They also shipped with a warning glyph on the decline and a primary capsule on the affirmative, which is the same over-claim in a channel no copy lock can see** — an exclamation mark and a colour saying what the words were written not to say. Both are now identically styled with no icon on either, held by one shared label builder so the symmetry is structural rather than two call sites agreeing. **And the two answers have different lifetimes:** per-session is the mash-through guard and is right for `true`, but `false` is a fact about one frame, so it is cleared after its capture and the question is owed again — persisting it would write "the examiner did not confirm" onto later photographs nobody was asked about. §1.3 of the spec is corrected in place, because it specified one button and the code followed it literally. (1) **`motionMeasurable` is now PERSISTED and `hasMotionBlur` has a note row — the trailing-window fix computed the distinction and nothing carried it.** One declaration, one doc mention, zero readers: `hasMotionBlur` is a plain `Bool` in evidence, so `false` meant both "measured and steady" and "never measured", and a gyro-less device produced the second while the report read the first. **A computed property with no reader is the always-false disguise with a better name.** Now on `CapturedPhoto`, decoded additively, written at `ScarCaptureView.performCapture` from the service's per-photograph answer. **And `hasMotionBlur` was being written into evidence with no note row at all — the mirror of §1.2's row with no writer**, both leaving the artefact unable to make a claim the tree supports. **§2.3's all-clear is now REPAIRED, not merely recorded, and the repair is a SECOND VARIANT rather than a softer sentence:** "met the app's capture-quality checks" covered a photograph whose motion was never measured as though its steadiness were verified — **a member missing a check, independent of the set missing a member**, which is why the earlier repair to that section did nothing for it. `allclear.partial` is emitted only when some analysis photograph has `motionMeasurable == false`, and is **locked** (§4.1). **Qualifying the single sentence permanently would put "the checks that could be run" on every report, where it distinguishes nothing and becomes furniture** — the always-firing note re-entering in the one section whose purpose is the quiet case. **And it is one clause per REPORT, never a note per PHOTOGRAPH:** §1's motion row has no "not measured" line deliberately, so a per-photo version would annotate every photograph on a gyro-less device and destroy the note's meaning for the one that needed it. Three wordings are forbidden with reasons, because each fails in a way no diff shows. Not fixed by treating `motionMeasurable == false` as a pass. **And the Tech Lead's row-count check fires against this patch, which is why it is recorded in the section rather than left to a reader: the lock now specifies TWO all-clear variants and `PDFReportGenerator` emits ONE literal from a single `!anyNotes` branch** — the sixth-row defect at the all-clear instead of at a note, with this document on the over-claiming side. The owed selector is stated checkably (`captureConditionPhotos(in:).contains { !$0.motionMeasurable }`, the same population helper, never a second enumeration). **A row with no branch and a condition with no writer are the same defect at opposite ends of the pipe**, and both read as complete from the side you are standing on. **And `check_note_rows_implemented` does not cover this section, measured rather than assumed: its population is the §2.2 Trigger/Note table's ROWS, and the two all-clear variants are BLOCK QUOTES** — so with `allclear.partial` specified and unrendered, a full `preflight --all` is clear and silent. **The check is correct and its enumeration does not reach the sentence the section is about**; the gap to close is the POPULATION (this document's locked strings wherever written) and not the check. That is the lock's own three-times-widened scope failure aimed at a check, since widened by its author to read both variants. **THE SELECTOR §2.3 OWED WAS ITSELF THE DEFECT and the Designer's hold is right — I verified the population at the source: `motionMeasurable` is written on ONE path with `false` as the struct default, and `CameraService`'s single `CapturedPhoto(` init passes it NOT AT ALL while constructing every protocol photo.** The capture population is `PhotoType.requiredCaptureProtocol` — **2 `closeupDamage` + 2 `paintTransfer` per vehicle, so FOUR analysis shots each and EIGHT across two vehicles**, plus the two scar photos. **Not the 30-step `CaptureProtocolStep.fullProtocol`, which is coaching metadata and whose own doc comment warns against reading it as the capture population — Vector corrected my twelve, and he had made the identical error hours earlier: twice in one day by two people is a recurring trap, not a slip, and it survived because the number was never load-bearing for the conclusion.** So `contains { !$0.motionMeasurable }` is true on every real case, the qualified form becomes the only reachable variant, and the plain sentence dies — **the always-firing qualification four paragraphs forbid, arriving through the SELECTOR instead of the WORDING.** §2.3 holds on the single plain variant; `allclear.partial` stays locked and **unemitted**, an honest missing claim over a wrong one on every report. Owed first: a per-path motion-capability field, the way `motionMeasurable` replaced `frameConfirmedClear != nil`; the narrow `&& $0.sharpnessMeasurable` fix is the proxy §1.1 removed. (3) **§6.1's FILTERED VERDICT IS SPECIFIED, `Required`, AND NOT BUILT — the Tech Lead's find, verified by me, and the only item on the board that puts a WRONG FORENSIC VERDICT on a page.** Both `headlineDisplay` definitions read the **unfiltered** `permutationPValue` and print "above chance"/"NOT distinguishable from chance" with no filtered branch, at four render sites, while `filteredSummary` suppresses the verdict correctly directly beneath — **so the loud headline contradicts the careful summary and defeats the suppression Prism's 15.6× false-positive measurement bought.** Neither of today's checks can see it: **the branch EXISTS and its guard is wrong by OMISSION**, which is a different class from a row with no branch and the only one still undetectable. Repair specified in §6.1 and implementable with no new model field (`exclusions` and `filteredOutcome` are already on the struct); it belongs in `headlineDisplay`, never in the call sites. **Scope corrected by the Designer and my count was wrong in the same class as the "twelve": TWO render sites can be wrong, not four** — `ScarFingerprintMatch` has no `exclusions` and no `filteredOutcome`, so there is no filtered state for it to misreport and its two sites are correct as they stand. **Verify a type HAS the feature before counting its render sites; a count of call sites is not a count of exposed ones.** **The Designer BUILT it and found a second defect in the same block plus the worse half of mine: `significanceColor` painted the headline GREEN — approval conferred with no word read, so the verdict was travelling by HUE through a channel string-level suppression cannot reach** (the `confirm.no` glyph class again). New `reportableSignificance` is `nil` under exclusions and **`nil` never `false`**, since orange would assert "tested and found insignificant" — the same defect sign-inverted. **And my §6.1 SCOPE RULING on her question: the locked template names the FILTERED figure while `headlineDisplay` carries the UNFILTERED score, so it cannot be used whole — but her string left a BARE percentage beside "for a filtered subset", and an unqualified quantity inherits its subject from the sentence next to it.** Locked first sentence now names the score's own subject ("Unfiltered comparison — NN% similarity across all cross-sections"); the claim-bearing second sentence is verbatim and unchanged; the original template stands where a surface carries the filtered figure. **Rule: when a locked template cannot be used whole because one figure is not the figure at hand, name the subject of the figure you DO have — never drop the qualifier, never keep a bare number.** **Landed verbatim by the Tech Lead (`40e4709`), together with §4.3.1's appended limit sentence and the heading moved inside the signed branch — both my rulings implemented and verified in place.** The Designer's own label patch is **fully superseded**: her proposed string is byte-identical to what landed. **Her error diagnosis is the durable part: she collapsed TWO decisions into one — declining to PARAPHRASE the template was right, declining to LABEL was a separate decision she never made, so she verified she had not altered locked copy and never asked what the number beside it now claimed.** **And her shape check's mutations were not independent (Tech Lead's find): reverting both guards failed 7, reverting the HEADLINE guard alone also failed 7, so the colour assertions never discriminated and the check would have passed a tree with only the colour guard removed — the exact state the hue defect lived in through three sign-offs. The mutation run was real and the discrimination was not.** Fixed with a section 6b that asserts the colour channel with the string HELD CORRECT; **I verified the independence by compiling and running it here rather than accepting the claim — colour guard alone fires 4 colour-exclusive assertions, headline guard alone 5 headline-exclusive, overlapping only on the shared total.** **New rules: mutate each half of a two-part fix separately and require each to fail assertions the other does not; a matching pair of test counts is a number that matches, not a number that agrees.** **AND THE MUTATION FINDING I ENDORSED WAS FALSE — Vector re-ran it, I re-ran it independently, and the Designer's v1 check WAS already isolating the colour guard.** Measured on the attached v1: **colour guard alone 3 failures (two the named colour assertions), headline guard alone 5 with no colour assertion, both 7 — 3 + 5 − 1 shared totalising assertion = 7 exactly. Disjoint from the start.** The artefact came from **mutating by PATTERN, not by location**: both guards match the text `guard !hasExclusions`, so one replace removes both and reproduces the double-revert count — I reproduced that too. **Three of us endorsed the correction within four minutes, I wrote it into PROCESS as fact, and the Designer rewrote a correct instrument on it. Nobody re-ran it — and MY OWN "verification" was the worst link: I compiled and ran her v2, which the false correction had already induced, so the chain never touched the disputed v1.** **A verification that does not load the artefact as it was when the claim was made is a verification of AGREEMENT**, and it reads as strong evidence precisely because it involved running real code. **Rule: name the version in the claim and in the answer.** And the failure direction is inverted from every other defect today (Tech Lead's): the rest made something **absent look fine**; **this made something fine look absent, and cost a working instrument** — a hunt tuned to one direction is blind to the other. **THE RULE IS KEPT AND ITS FALSE EXEMPLAR REMOVED: a correction can be wrong while everyone's agreement makes it feel verified, and an unverified correction spreads faster than the defect it names because agreeing with a self-criticism feels safe — nobody re-runs a claim someone has volunteered against themselves.** New companion rules: **mutate the guard, never the model the guard reads; mutate by location, never by pattern.** Her 6b rewrite is kept **on its merits, not as a repair** — the cross-channel relation (*the loudest signal must not disagree with the words beside it*) is strictly stronger than v1, and "mutation-verified" was never the false claim. (5) **SHAPE-CHECK HOME RULED (mine, a manifest-and-lock question the Tech Lead correctly refused to land silently), and RECONCILED to `scripts/shapechecks/*.shapecheck` with a mutation-tested `run.sh` — both the Designer and I built a home from the ruling and it offered two acceptable forms, which is why two landed. **`.shapecheck` is the better of them: a distinct extension reads as a deliberate class rather than a Swift file someone renamed.** Seven checks in the directory; I ran the runner (**7/7 pass**) and independently verified the empty-directory case returns **rc=2** rather than a phantom failure. **And the reconciled tree is now measurably right in the way the ruling intended: the scoped and unscoped Swift counts BOTH read 42, so the two figures quoted as one actually are one.** **The Designer corrected my mechanism and I corrected hers, in opposite directions:** she claimed a `.swift` file under `scripts/` would move "42/42 parsed" — it would not, `tracked_swift()` is scoped; I credited that scoping as the fix when **the figure that would MOVE is the manifest total, whose check counts every tracked `.swift` with NO scoping at all, so the EXTENSION was the whole fix.** **A correct diagnosis citing the wrong mechanism produces a heavier repair than the defect needs**: I proposed two forms where one suffix sufficed, and both got built. **Her own caution is the sharper one: she had been counting `git ls-files '*.swift'` UNSCOPED all day, so her "42/42" and preflight's were the same number from different populations — name the population, not just the number.** A verification instrument is not app source. **The Tech Lead reconciled both homes rather than picking one — correctly, since my ruling explicitly allowed either form, so neither patch was wrong — and renamed my stored v1 to `.shapecheck` so the runner actually executes it: a stored instrument the runner does not pick up is a file, not a check.** He also caught his own second measurement error before publishing, and it belongs with the other two: **reading `bash run.sh | tail; echo RC=$?` reports `tail`'s status, not the runner's**, which showed rc=0 on a deliberately broken check. **All three of the round's measurement errors — mutating by pattern, mutating the fixture, reading a status through a pipe — return a plausible integer with no error anywhere, which is why looking harder at the result catches none of them.** **And one I found by running the runner bare, fixed in the commit that recorded it: `run.sh` RETURNED rc=2 for BOTH "no shape checks found" and "no `swiftc` on PATH".** The printed lines distinguished them; **the exit code did not, and an exit code is what a caller reads** — so "the checks did not execute" was indistinguishable from "there is nothing to execute", which are opposite problems. The absent-toolchain case now returns **rc=3**: 0 pass / 1 ran-and-failed / 2 nothing-to-execute / 3 could-not-execute, documented in the runner and its README and verified bare, one condition at a time. **And the Designer's half of the repair is the better one: the verdict also goes to STDERR, because stderr survives a stdout pipe** — `| tail` is the natural way to read seven ok lines and it discarded the only machine-readable signal while the text still said "1 failing". **Cheaper to stop rewarding the mistake than to write a rule nobody re-reads:** a rule guards the reader who remembers it, a redundant channel guards the one who does not. `Totals:` names both figures explicitly and states that both 42s are the same 42, with the shape-check exclusion stated in the prose and deliberately WITHOUT a file count -- the set grows, and a count there would go stale in the direction that reads as an oversight. **A figure whose population changes without its sentence changing is the same defect as a locked string whose meaning changed without its characters changing (`confirm.arm`) — a diff of a count shows its value, never its population.** **NEW EXPOSURE my ruling created, recorded in §6.1 rather than left implicit: labelling the headline put TWO labelled percentages on one page where before there was one mislabelled one, and whether they are distinguishable at a glance is a judgement about rendered pixels no check here can make. If a device shows it, the repair is the LABELS' prominence and never the numbers.** (4) **§4.3.1's attestation was in LIVE LOCK CONFLICT and I ruled it — neither side adopted whole.** The tree is third-person about capture conditions, the lock first-person about generating the report, and the code dropped the identification limit. **The code's SUBJECT is right, the FIRST PERSON was my defect** (the app generates the report and then put that sentence in a person's mouth — §4.0.1 broken by the attestation itself), and **the identification limit returns with *the report* as its subject.** New locked `attest.body` with a ledger row. **On the unsigned branch the document stands and the code's citation of the specify-omission rule is a misreading:** the block still draws its **"Attestation"** heading, so the page carries an attestation section whose content is that there is no attestation — the absence asserting something with a title on it. Omit the block entire, heading included. The gyro was measured at 30 Hz all along; what `hasMotionBlur` lacked was a WINDOW, not a sensor — a photograph is blurred by movement during the exposure, not at the instant the gate last recomputed. Now written from the peak rotation-rate over a **trailing** window **ending at the shutter**, against the gate's own threshold, and `false` on a device with no gyro where nothing was measured. **Corrected once in place (Vector): the first version opened that window at `armAutoCapture()`, which the manual shutter never calls** — §2.4 keeps it enabled on failing gates deliberately — **and `resetAutoCaptureStreak()` disarms after every capture without clearing the peak, so an unarmed manual shot and a second manual shot both reported a peak measured before the frame existed.** Verified by construction on a reduced shape that runs, not by reading the diff. **A window opened by a control cannot bind a path that skips the control**; a trailing window needs none. `motionMeasurable` is a separate field from `motionMeasured` and not a duplicate of it — the latter is a property of the device, the former of this photograph, because wall-clock advances with no gyro callback and **silence is not steadiness**. **`issueDescriptions` still cannot express "unmeasured" by being short**, which is unchanged by this diff and is why it still has zero readers. `isTooClose` stays named-unassigned: the fill gate measures edge-energy concentration and cannot separate "fills the box" from "nearer than the lens can focus". (2) **THREE provisional uncalibrated thresholds now, not two, and the third carries a live risk worth naming:** the two gate thresholds plus `rotationRateThreshold` = 0.12 rad/s, which `hasMotionBlur` now reuses over a ~0.65 s hold. **Nobody has measured whether an ordinary handheld peak clears 0.12 rad/s over that window, and if it routinely does, `hasMotionBlur` becomes the always-firing note this morning was spent removing** — the same defect re-entering through a threshold instead of through a predicate. **Failure direction: too LOOSE annotates nothing and loses a real finding; too TIGHT annotates every photograph and destroys the note's meaning for the one that needed it.** Reusing the gate's own figure is right regardless of its value (the report must not contradict the gate about the same frames), so calibration moves the pair together — Prism's, with the other two, and it is the only one of the three whose miscalibration degrades an evidence artefact rather than the capture experience. **And `measuredMotionBlur` is a THIRD instance of the half-published class, the strongest yet: it reads `motionMeasured` and the rolling `motionSamples` buffer — `peakRotationRate` before the trailing-window fix deleted it — and NEITHER is `@Published`** — so unlike the pair below, a `body` reading it would not refresh at all. Harmless only because its single reader is imperative. Three in one service means the shape is the service's, not the properties': every new `measured*` computed property here starts unobservable by default. (3) §2.4's per-factor requirement stays recorded as **unmet** rather than rewritten to match what shipped, and the struck on-device item is re-enabled by the diff that adds the photo linkage — at which point the added subject sentence becomes the imprecise one and should go. **A spec edited to agree with its implementation stops being able to disagree with it.**
